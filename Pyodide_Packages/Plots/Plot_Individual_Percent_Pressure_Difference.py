...
Figure_Basename = "individual_percent_pressure_difference"
...
# Build the individual-panel percent pressure-difference figure and return it
def Create_Figure(Dataset, Config_Module):
    ...
    if not Eos_List_Filtered:
        raise ValueError("No individual EoS panels are available for percent pressure-difference plotting.")
    ...
    # Build the y-axis label from the reference study name; falls back to generic form
    ...
    for Eos_Index, Eos in enumerate(Eos_List_Filtered):
        ...
        Plot_Styling.Style_Ax(Axis, X_Pressure_Label, Y_Label, f"{Study_Label}\n{Tail_Label}")
    ...
    Plot_Styling.Place_Spread_Suptitle(
        Figure,
        Axes_List,
        f"{Dataset['inputs']['composition']} - Individual EoS (Pdiff/P%)\n{Plot_Styling.Sym_Note}",
        Gap_Lines=2,
    )
